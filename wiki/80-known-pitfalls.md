...
现象：一次 Ctrl/⌘K 在 DOM 中产生两个搜索弹窗，按一次 Escape 仍残留一个。
原因：桌面与手机各挂一份 CommandMenu，CSS 的 hidden 不会卸载组件或移除键盘监听。
解法：AppShell 只挂载一个受控命令面板，两个可见性互斥的入口共享打开状态。
教训：覆盖 DOM 中的弹窗数量和关闭后的残留状态；仅按可访问角色查询会漏掉被另一层 aria-hidden 的弹窗。

## 模型评估不能阻塞完成记录（2026-09-19）

现象：用户已读完并交卷，点击完成后要等 profile 请求，断网则课程仍未完成。
原因：本地事实和模型推断写在同一个 await 之后。
解法：先原子记录本地完成事实与答案快照，独立评估，失败可重试；每目标串行、按任务 ID 提交。
教训：刷新、重复点击、路径重排和手动编辑画像都要覆盖；重试不能改变 completed_at，迟到结果
不能覆盖新画像。只验证正常请求成功会漏掉这类学习闭环问题。

## 2026-09-19：评分、复习与窄屏可访问性

- **损坏答案被部分接受**：`parseInt` 接受带后缀的数字，多选过滤掉无效正确项后可能误判。
  现在要求完整数字索引和完整有效答案集合；缺失选择统一为未答。教训：解析失败不能缩小正确答案集合。
- **简答漏答永久挂在复习队列**：只认 correct 会让补答后的简答一直待处理。已补答简答退出漏答队列，
  同时保留待自评说明，不把 answered 升为 correct。教训：行动完成与客观评分是两个状态。
- **窄屏关系图无法键盘滚动**：视觉上可横滑，但 axe 报 scrollable-region-focusable。
  滚动容器增加命名 region、tabIndex 和焦点样式。教训：每个独立滚动区域都需检查键盘入口。

## 创建入口保存配置时不能卸载对话框入口

现象：连接配置保存成功后，原按钮被条件渲染移除，关闭弹窗无法返回入口焦点。
原因：用“未配置”同时控制整个 ModelSettings 的挂载和提示。
解法：始终保留同一实例与按钮，只改配置状态和按钮文字，增加关闭后焦点断言。
教训：成功状态不能破坏承载当前键盘焦点的组件；异步检查也要防止卸载后启动流程。
