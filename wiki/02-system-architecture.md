...
全局命令面板由 AppShell 持有唯一打开状态，桌面和手机按钮共享同一个面板及键盘监听。
目标选择器复用 archive 的 active_goal_id；切换目标时进入新目标的路径页，避免沿用旧目标的
课程索引。导师仍按 goal.id 隔离，新目标不会显示旧目标的草稿或消息。

### 本地完成与技能评估分离（2026-09-19）

`store/completion.ts` 原子写入 `if_learned`、`completed_at` 与测验评估任务。没有答案时只记录
完成，不伪造掌握度。`SessionState.profile_update` 是可选字段，含唯一任务 ID、pending/failed
状态和完成时的答案快照；随 archive v1 导出导入，旧档案仍有效。网络运行状态仅在内存中。
AppShell 提示已保存，并自动处理当前目标的 pending；失败后必须手动重试，不循环请求。
同一目标按完成顺序串行评估，每次使用最新画像。结果按任务 ID 找回课程（允许路径重排），
一次性清除任务并记录掌握度；不会重复计时。请求期间手动修改画像时保留新画像，评估转为可重试。
删除目标后的迟到结果直接丢弃。刷新期间未完成的 pending 会在下次打开该目标后恢复。

阅读区选中文字后可通过“请教选中内容”把最多 4000 字带入导师草稿；追加而非替换已有草稿，
不自动发送。引用属于用户可编辑的消息内容，仍通过现有 Tutor API；不新增 prompt 或接口。

### 可解释进展与专项练习（2026-09-19）

`store/learning-evidence.ts` 以已确认的技能要求为分母，每项技能等权；名称经 Unicode、空白和
大小写归一化，重复要求取更高目标，冲突画像取较低估计。画像缺项时回退到确认过的起点。
目标、画像、进展和技能关系共用这一派生结果；不再把模型任意填写的 `overall_progress` 当作
当前达成率。模型原字段与历史快照保留兼容，历史曲线明确标注为画像估计。

`quiz-review.ts` 从首次测验的 incorrect / unanswered 生成专项练习并保留新旧题号映射。
复习队列按错题数、漏答数降序、首次测验时间升序排列，同分按原课序；不声称是遗忘曲线算法。
`SessionState.practice` 可选保存来源提交时间、练习草稿与结果，沿用 archive v1，可导出导入。
来源时间不匹配的练习不抵扣当前队列；答对的客观题和已经补答的简答题退出队列。简答仅待自评，
不自动判对。练习永远不覆盖首次成绩、课程完成时间或掌握度历史，也不触发画像升级请求。

缺失单选/判断选择明确计为未答；多选答案键必须全部可解析且非空才能自动评分，数字索引
必须是完整数字字符串，不能将损坏答案部分解析后判对。无效答案键保留为待自查。

### 首次访问的配置前置检查（2026-09-19）

GoalForm 在本地字段校验后检查模型配置：设备 BYOK 或服务端配置存在才开始目标分析，
replay 模式供测试离线执行。配置缺失打开受控 ModelSettings，配置检查失败保留草稿并允许
重试。保存配置不会自动发起模型请求。组件卸载后忽略晚到检查，不启动已经离开的创建流程。
检查只代表存在配置，不宣称模型连接已验证。首页的演示路径独立于档案和创建状态。

## 模型目录（2026-09-19）

模型设置内两个选择器共用一份临时目录，由 `use-model-catalog` 经 `api.models` 请求
