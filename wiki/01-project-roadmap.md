...
| 功能 | 位置 | 状态 |
|---|---|---|
| 单实例快捷搜索、目标快速切换、短屏与焦点恢复 | `components/layout/`、`components/ui/command.tsx` | done |
| 固定导师时的容器响应式目标、文库与画像 | `features/goals/`、`features/library/`、`features/profile/` | done |
| 导师长草稿、长代码与回到最新消息 | `features/tutor/` | done |

## 学习闭环深度迭代（2026-09-19）

| 功能 | 状态 |
|---|---|
| 当前课目标水平、真实继续状态与主行动层级 | done |
| 阅读常驻工具栏、引用选中内容到导师草稿 | done |
| 本地完成先保存、技能评估独立恢复与并发保护 | done |

## 产品、设计与算法三轮迭代（2026-09-19）

| 功能 | 状态 |
|---|---|
| 已确认技能目标派生当前达成率，跨页面统一口径 | done |
| 可解释错题优先队列，首页与技能记录直达相关课程 | done |
| 独立专项练习、草稿恢复、首次成绩保护与档案兼容 | done |
| 进展页行动层级、双主题窄屏布局、键盘滚动与焦点 | done |
| 无效答案键、缺失选择、简答自评与重复技能边界修复 | done |

## 首页与创建流程精修（2026-09-19）

| 功能 | 状态 |
|---|---|
| 清晰价值说明、可切换示例路径、已存草稿继续入口 | done |
